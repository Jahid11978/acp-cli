...
| `acp agent tokenize --launchpad occupy --quote-token <address> [--name <name> --pool-fee --tax-bips --no-thicken-liquidity --anti-sniper <0\|1> --prebuy]` | Launch on the **Occupy** launchpad instead: one transaction, **no launch fee and no VIRTUAL needed** (gas is sponsored), curve quoted in `--quote-token`, which is **required, has no default, and takes an ADDRESS — not a ticker**. There is no command that lists the assets; the allow-list is published at <https://os.virtuals.io/agent-identity/token/overview#occupy-quote-assets> and you must read it from there to get an address. `NVDAc` is `0xb20000000000000000000078ee7ce2fE4908108C`. It is a **tokenized equity**, not VIRTUAL or a stablecoin, so it decides which stock the agent token trades against. Surface that choice to the human like any other launch economics. EVM only. `--acf`/`--60-days`/`--airdrop-percent`/`--robotics` are Virtuals-only and are rejected here. A `--prebuy` is denominated in the quote asset, so it requires `--quote-token`. `--name` sets the token name (Occupy names the token independently of the agent; Virtuals does not, and `--name` is rejected there), and `--anti-sniper` on Occupy offers only `0` (none) or `1` (60s) — `2` is Virtuals-only. The three pool flags are **permanent AMM economics, not cosmetics** — set them deliberately or leave them at their defaults, and say which you did: `--pool-fee` is the Uniswap v4 swap fee in hundredths of a bip, default `10000` (1%), bounded on-chain to `10000`–`30000` (1%–3%) — every trade in the token pays it, so it is a permanent tax on the token's own liquidity; `--tax-bips` is **draft metadata, not part of the launch transaction** — the deployed Base `Bonding.launch` takes no tax parameter, so do NOT tell a human this sets a trading tax; leave it at its default unless they ask, and say it is unverified if they do; `--no-thicken-liquidity` disables liquidity thickening, which is **on by default** — thickening concentrates the launch liquidity to reduce early price impact, so turning it off makes the opening price move more per trade. All three are Occupy-only and rejected on the Virtuals launchpad. |
