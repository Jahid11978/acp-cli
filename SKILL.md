...
| `acp agent tokenize --launchpad occupy --quote-token <address> [--name <name> --pool-fee --take-fees --anti-sniper <0\|1> --prebuy]` | Launch on the **Occupy** launchpad instead: one transaction, **no launch fee and no VIRTUAL needed for the launch itself** (but the wallet still needs a little USDC, USDT or VIRTUAL to pay the network fee — gas is charged in an ERC-20, not ETH, so a completely empty wallet fails with "Not enough balance to cover the network fee"), curve quoted in `--quote-token`, which is **required, has no default, and takes an ADDRESS — not a ticker**. There is no command that lists the assets; the allow-list is published at <https://os.virtuals.io/agent-identity/token/overview#occupy-quote-assets> and you must read it from there to get an address. `NVDAc` is `0xb20000000000000000000078ee7ce2fE4908108C`. It is a **tokenized equity**, not VIRTUAL or a stablecoin, so it decides which stock the agent token trades against. Surface that choice to the human like any other launch economics. EVM only. `--acf`/`--60-days`/`--airdrop-percent`/`--robotics` are Virtuals-only and are rejected here. A `--prebuy` is denominated in the quote asset, so it requires `--quote-token`. `--name` sets the token name (Occupy names the token independently of the agent; Virtuals does not, and `--name` is rejected there), and `--anti-sniper` on Occupy offers only `0` (none) or `1` (60s) — the contract would accept `2` (98 min) but Occupy does not offer it and the API rejects it; it is available on the Virtuals launchpad. **Fee economics — surface both of these to the human, they are permanent.** `--pool-fee` is **the trading fee every buy and sell of the token pays**, default 1%. Occupy offers **exactly three rates: 1%, 2%, 3%** — there is nothing in between, so do not offer a human 1.5%. Pass the percentage (`1`, `2`, `3`) or the contract unit (**1% = `10000`, 2% = `20000`, 3% = `30000`**); both forms work. `100`, `200`, `1000` are NOT percentages — they are below the contract floor and rejected — so never hand-convert into bips. That fee is split globally by the launchpad (read from `AssetConfig.feeSplit()` on Base: creator 3000 bips = **30%**, quote asset's tax vault 4000, protocol 3000) — the split is not chooseable and not in the launch call. `--take-fees` decides what happens to the creator's 30%: it is **off by default, and off leaves that cut in the pool as permanent liquidity instead of paying it out**. Pass `--take-fees` to have it paid to the agent wallet as it accrues. Do not leave this to the default silently — a human launching a token usually assumes they receive their fees, and the default is the opposite. `--no-thicken-liquidity` is an alias for `--take-fees` (`thickenLiquidity` is the on-chain name; Occupy's own UI presents it as a take-fees switch). There is no trading-tax flag: the deployed Base `Bonding.launch` takes no tax parameter. All of these are Occupy-only and rejected on the Virtuals launchpad. |
